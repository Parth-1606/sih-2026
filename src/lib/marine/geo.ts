// Pure geospatial math. Coords are [lat, lng] unless noted.
...
export function destPoint(lat: number, lng: number, bearing: number, km: number): LatLng {
  const R = 6371;
  const d = km / R;
...
function segSeg(a1: LatLng, a2: LatLng, b1: LatLng, b2: LatLng): boolean {
...
export function routeIntersectsPolygon(route: LatLng[], poly: LatLng[]): boolean {
  ...
  for (let i = 0; i < route.length - 1; i++) {
    for (let j = 0; j < poly.length; j++) {
      if (segSeg(route[i], route[i + 1], poly[j], poly[(j + 1) % poly.length])) return true;
    }
  }
  return false;
}
